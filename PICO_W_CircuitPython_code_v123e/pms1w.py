...
def html_index_page():
    ...
    htmldata += '<p>  <a href="http://kll.engineering-news.org/kllfusion01/articles.php?article_id=213" target="_blank" ><b>kll.engineering-news.org</b></a> </p>'
    htmldata += '</body></html>'
    return htmldata
...
def setup_webserver() :
    ...
    server = HTTPServer(pool)  # ____________________ make a WEB SERVER
    ...
    def osci(request):  # pylint: disable=unused-argument
        ...
        #return HTTPResponse(content_type=MIMEType.TYPE_HTML, body=html_osci_page(rates[0]) )
        response = HTTPResponse(request)
        with response:
            response.send(body=html_osci_page(rates[0]), content_type=MIMEType.TYPE_HTML)

    @server.route("/osci1") # [>>>>]
    def osci(request):  # pylint: disable=unused-argument
        dp("\nwifi served dynamic osci.html")
        #return HTTPResponse(content_type=MIMEType.TYPE_HTML, body=html_osci_page(rates[1]) )
        response = HTTPResponse(request)
        with response:
            response.send(body=html_osci_page(rates[1]), content_type=MIMEType.TYPE_HTML)

    @server.route("/osci2") # [>>>]
    def osci(request):  # pylint: disable=unused-argument
        dp("\nwifi served dynamic osci.html")
        #return HTTPResponse(content_type=MIMEType.TYPE_HTML, body=html_osci_page(rates[2]) )
        response = HTTPResponse(request)
        with response:
            response.send(body=html_osci_page(rates[2]), content_type=MIMEType.TYPE_HTML)

    @server.route("/osci3") # [>>]
    def osci(request):  # pylint: disable=unused-argument
        dp("\nwifi served dynamic osci.html")
        #return HTTPResponse(content_type=MIMEType.TYPE_HTML, body=html_osci_page(rates[3]) )
        response = HTTPResponse(request)
        with response:
            response.send(body=html_osci_page(rates[3]), content_type=MIMEType.TYPE_HTML)

    @server.route("/osci4") # [>]
    def osci(request):  # pylint: disable=unused-argument
        dp("\nwifi served dynamic osci.html")
        #return HTTPResponse(content_type=MIMEType.TYPE_HTML, body=html_osci_page(rates[4]) )
        response = HTTPResponse(request)
        with response:
            response.send(body=html_osci_page(rates[4]), content_type=MIMEType.TYPE_HTML)

    @server.route("/osci5") # [slow]
    def osci(request):  # pylint: disable=unused-argument
        dp("\nwifi served dynamic osci.html")
        #return HTTPResponse(content_type=MIMEType.TYPE_HTML, body=html_osci_page(rates[5]) )
        response = HTTPResponse(request)
        with response:
            response.send(body=html_osci_page(rates[5]), content_type=MIMEType.TYPE_HTML)


    @server.route("/osci_details")
    def osci_details(request):  # pylint: disable=unused-argument
        dp("\nwifi served dynamic osci_details.html")
        #return HTTPResponse(content_type=MIMEType.TYPE_HTML, body=html_osci_details_page() )
        response = HTTPResponse(request)
        with response:
            response.send(body=html_osci_details_page(), content_type=MIMEType.TYPE_HTML)

    server.start(str(wifi.radio.ipv4_address)) # ____ startup the server
