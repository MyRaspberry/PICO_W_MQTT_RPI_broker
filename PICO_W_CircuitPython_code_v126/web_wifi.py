...
print(f"+ from ipaddress import ip_addre {gc.mem_free()}")
import wifi
...
useWEB = False #True
useWEB = bool(os.getenv('useWEB'))


# ____________________________________________________ expect /lib/adafruit_httpserver/*
# ____________________________________________________ CP802 to CP8282 delete 'Http'
if useWEB :
    from adafruit_httpserver import Server, Request, Response, POST
    print(f"+ from adafruit_httpserver import Server, Request, Response {gc.mem_free()}\n")
...
if ( get_useMQTT() ) :
    ...
    print(f"+ import adafruit_minimqtt.adafruit_minimqtt as MQTT {gc.mem_free()}")
    import json
    print(f"+ import json {gc.mem_free()}")
    import ssl
    print(f"+ import ssl {gc.mem_free()}")
...
if useWEB:
    ...
    def html_index_page():
        ...
        htmldata =  '<link rel="icon" type="image/x-icon" href="favicon.ico">'
        htmldata += HtmlStyle
        htmldata += '</head><body>'
        ...
        htmldata += '<hr></br><a href="/data" target="_blank" ><b>data</b></a> <hr>'
...
from mqtt import get_useMQTT, get_mqtts, get_MQTT_count, get_useFilter_Tuning, set_useFilter, set_filter_Va, set_filter_Ia, set_filter_Wa # check if we need MQTT inhere ( files: job5 mqtt )
from jobs import set_update5,get_JOB0en
JOB0en=get_JOB0en()
...
def setup_webserver() :
    ...
    if useWEB:
        ...
        def data(request):  # pylint: disable=unused-argument
            ...
            return Response(request, html_data_page(), content_type='text/html' )
        #  if a button is pressed on the site KLL change CP282 POST to "POST" ??
        ...
        def buttonpress(request: Request):
            ...
            with response:
                response.send(response, html_data_page() )


        server.start(str(wifi.radio.ipv4_address)) # _________ startup the server
        print(f"+ server.start {gc.mem_free()}")
        gc.collect()
...
def run_webserver() :
    ...
    if ( get_useMQTT() and MQTTok  ) :
        try:
            if JOB0en :
                # v1.2.6 test disable for timing issues by NOT JOB0en works, mqtt send and 1Mloop 70 sec
    ...
    if ( JOB0en ) : mqtt_DIO_change_send() # ___________________________ only on change by picow_io # v1.2.6 test disable
